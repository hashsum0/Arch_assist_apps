...
function inet_test {        
        ...
        echo -e "\x1B[32m""Здесь будет проверено соединение с интернетом""\x1B[0m"
        echo -e "\x1B[32m""с помощью команды ping -c3 8.8.8.8""\x1B[0m"
            echo "Продолжить?  y/N"
                read var
                    if [ "$var" == "y" ]
                    then
...
function parted_disk {
        ...
        echo -e "\x1B[36m""sd*1 для boot(150M) sd*2 для корневого раздела / (8G+ желательно 25G+) ""\x1B[0m"
        echo -e "\x1B[36m""sd*3 для swap(размер ОЗУ+) и sd*4 для раздела /home (обычно все остальное) ""\x1B[0m"
...
function formating_disk {
        ...
        echo -e "\x1B[36m""с помощью команды mkfs.ext2 /dev/sd*№ для boot""\x1B[0m"
        echo -e "\x1B[36m""mkfs.ext4 /dev/sd*№ для / и mkfs.ext4 /dev/sd*№ для home""\x1B[0m"        
            echo "Продолжить?  y/N"
                read var
                    if [ "$var" == "y" ]
                    then
